    ...
    void Problem::drawLabels (std::ofstream &svgmap) {
        ...
        for (i = 0;i < nbft;i++) {
            if (sol->s[i] >= 0) {
                ...
                toSVGPath (4, 3, lp->x, lp->y, pal->getDpi(), scale, pal->map_unit, convert2pt (bbox[0], scale, pal->getDpi(), pal->map_unit, bbox[2]-bbox[0]), convert2pt (bbox[2], scale, pal->getDpi(), pal->map_unit, bbox[2]-bbox[0]), convert2pt (bbox[3], scale, pal->getDpi(), pal->map_unit, bbox[2] - bbox[0]), "label", lp->feature->uid, svgmap);
            }
        }
